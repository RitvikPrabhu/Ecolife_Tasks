...
class tech:
    ...
    def optimize(self):
        ...
        ]  # this is used for the past window size invocation interval
        old_warm_pool = {}
        new_warm_pool = {}
        ...
        for j in range(self.window_size, self.window_size + self.interval):
            ...
            for i in range(function_num):
                ...
                interval = [
                    invoc_index[i + 1] - invoc_index[i]
                    for i in range(len(invoc_index) - 1)
                ]
                invoke_interval[i].append(interval)

                function_name = self.trace_function_names[i]
                old_cold_st, old_warm_st = utils.get_st(
                    function_name, self.server_pair[0]
                )
                new_cold_st, new_warm_st = utils.get_st(
                    function_name, self.server_pair[1]
                )
                cold_carbon, warm_carbon = utils.compute_exe(
                    function_name, self.server_pair, self.carbon_intensity[j]
                )
                ...
                if concurrent_function == 0:
                    ...
                    if i in old_warm_pool:
                        if int(old_warm_pool[i]["end_time"]) <= j:
                            ...
                            last = int(old_warm_pool[i]["end_time"]) - int(
                                old_warm_pool[i]["start_time"]
                            )
                            kat_carbon = int(
                                old_warm_pool[i]["num"]
                            ) * utils.compute_kat(
                                ...
                                self.carbon_intensity[
                                    int(old_warm_pool[i]["start_time"])
                                ],
                            )
                    ...
                    if i in new_warm_pool:
                        if new_warm_pool[i]["end_time"] <= j:
                            last = int(
                                new_warm_pool[i]["end_time"]
                                - new_warm_pool[i]["start_time"]
                            )
                            kat_carbon = int(
                                new_warm_pool[i]["num"]
                            ) * utils.compute_kat(
                                ...
                                self.server_pair[1],
                                last,
                                self.carbon_intensity[new_warm_pool[i]["start_time"]],
                            )
                            sum_carbon += kat_carbon
                    ...
                    # execute function:
                    st_per_func, carbon_per_func, result_st[i], result_carbon[i] = (
                        exe_decide.exe_loc_decision(
                            old_warm_pool,
                            new_warm_pool,
                            i,
                            concurrent_function,
                    ...
                    if i not in new_function:
                        # first invoke set the hc optimzier
                        parameters = [self.hc_size, self.kat_time, self.st_lambda]
                        new_function[i] = hill_climbing.HillClimbing(
                            parameters,
                            self.server_pair,
                            function_name,
                            self.ci_avg,
                            self.carbon_intensity[j],
                            invoke_interval[i][j - self.window_size],
                        )
                        a, _ = new_function[i].main(
                            self.carbon_intensity[j],
                            invoke_interval[i][j - self.window_size],
                        )
                        decision = a
                        ...
                        decision, b = new_function[i].main(
                            self.carbon_intensity[j],
                            invoke_interval[i][j - self.window_size],
                        )
                        ka_loc = int(decision[0])
            ...
            mem_checker = utils.mem_check(
                self.mem_new_limit,
                self.mem_old_limit,
                old_decision,
                new_decision,
                self.function_mem_trace,
                new_warm_pool,
                old_warm_pool,
            )
            if mem_checker == 0:
                ...
                for key, value in new_decision.items():
                    new_warm_pool[key] = value
                old_warm_pool, left_pool, kat_carbon, result_carbon = utils.adjust_pool(
                    self.mem_old_limit,
                    self.function_mem_trace,
                    old_warm_pool,
                    old_decision,
                ...
                sum_carbon += kat_carbon
                new_warm_pool, kat_carbon1, discard, result_carbon = (
                    utils.add_discard_pool(
                        new_warm_pool,
                        left_pool,
                        self.mem_new_limit,
                        self.function_mem_trace,
                ...
                for key, value in old_decision.items():
                    old_warm_pool[key] = value
                new_warm_pool, left_pool, kat_carbon, result_carbon = utils.adjust_pool(
                    self.mem_new_limit,
                    self.function_mem_trace,
                    new_warm_pool,
                    new_decision,
                ...
                sum_carbon += kat_carbon
                old_warm_pool, kat_carbon1, discard, result_carbon = (
                    utils.add_discard_pool(
                        old_warm_pool,
                        left_pool,
                        self.mem_old_limit,
                ...
                old_warm_pool, left1, kat_carbon1, result_carbon = utils.adjust_pool(
                    self.mem_old_limit,
                    self.function_mem_trace,
                    old_warm_pool,
                    old_decision,
                ...
                new_warm_pool, left2, kat_carbon2, result_carbon = utils.adjust_pool(
                    self.mem_new_limit,
                    self.function_mem_trace,
                    new_warm_pool,
                    new_decision,
            ...
            print(
                f"current service time is:{sum_st/sum1}, carbon is: {sum_carbon/sum1}"
            )
        print(f"service time is:{sum_st/sum1}, carbon is: {sum_carbon/sum1}")
        with open(
            f"{Path(__file__).parents[1]}/results/hill_climbing/carbon.json", "w"
        ) as file:
            json.dump(result_carbon, file, indent=4)
        with open(
            f"{Path(__file__).parents[1]}/results/hill_climbing/st.json", "w"
        ) as file:
