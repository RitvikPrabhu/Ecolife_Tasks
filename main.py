...
import utils
from optimizers import carbon_opt, oracle, perf_opt, tech, tech_hc
...
def main(
    ...
    carbon_intensity, ci_max, ci_min, ci_avg = utils.load_carbon_intensity(
        region, start_hour, interval
    )
    ...
    function_mem_trace = [
        utils.read_func_mem_size(trace_function_names[i]) for i in range(len(traces))
    ]
    ...
    if optimizer == "perf_opt":
        ...
        optimizer = carbon_opt.carbon_opt(
            traces,
            trace_function_names,
            server_pair,
            carbon_intensity,
        ...
        optimizer.optimize()
    elif optimizer == "hill-climbing":
        optimizer = tech_hc.tech(
            traces,
            trace_function_names,
            server_pair,
            kat_time,
